...
# importing shapefile for adm1 borders 
brn_adm1 <- read_sf("assign_1/gadm_brn/gadm36_BRN_1.shp")
...
ggplot(brn_adm2) + 
  ...
  geom_sf(data = brn_adm1,
          color = "midnightblue", 
          size = 1, 
          alpha = 0) + 
  geom_sf_text(data = brn_adm1, 
               aes(label = NAME_1),
               nudge_x = -0.01,
               nudge_y = -0.02,
               color = "black",
  ...
  geom_sf_text(aes(label = NAME_2),
               size = 3) + 
  scale_fill_gradient2(low = "darkslategray3", mid = "darkorchid3", high = "maroon2", midpoint = 40000) +
  labs(title = "Brunei's Districts", subtitle = "Population Heatmap", x = "longitude", y = "latitude") + 
